...
for (i in 1:nrow(ten_km_grid)) {
  ...
  ## If there were no roads downloaded, it can be removed  
  rm(temp_road)
  
  if (exists('grid_roads')) {
    
          # Crop the roads to the exact box rather than the bbox
         
          
          print(paste0('There were ', nrow(grid_roads), ' roads in this grid'))
          ...
          grid_roads[nchar(grid_roads$maxspeed) == 6 & !is.na(grid_roads$maxspeed),'maxspeed'] <- substr(grid_roads[nchar(grid_roads$maxspeed) == 6 & !is.na(grid_roads$maxspeed),]$maxspeed,1,2)
  ...
  # Having got all the roads in this grid square, and harmonised, them, crop to the exact grid square as was just the bbox before
